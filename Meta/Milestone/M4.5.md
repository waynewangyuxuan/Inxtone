...
Make the writing workspace *intelligent* — the Bible panel becomes a live co-pilot, search makes content discoverable, and AI-generated text feeds back into the Story Bible automatically. After M4.5, writing in Inxtone feels like writing *with a partner who remembers everything*.

**Product pivot**: Inxtone focuses on **writing from scratch** (not importing existing long works). Intake scenarios assume the writer is passing in outlines, character designs, and world designs — not 1000-chapter backlogs.

## Success Criteria

```
✓ Cmd+K search modal with instant full-text results across all entities
✓ Interactive Bible panel shows entity details, supports inject-to-context
✓ Relationship map visualization (force-directed graph)
✓ Timeline visualization (chronological event display)
✓ Pacing visualization (word count / emotion curve per chapter)
✓ Chapter setup assist auto-suggests relevant entities from outline + arc
✓ Post-accept extraction: AI-generated text → new/mentioned entities detected
✓ Keyboard shortcut system covers all frequent actions
```
...
2. **Interactive Bible Panel (P0)** — Issue #34
   - Click entity → expand inline detail (full profile, not just name + badge)
...
4. **Timeline Visualization (P0)**
   ...
   - Click event → navigate to chapter/entity
...
6. **Chapter Setup Assist (P0)** — Heuristic Layer
   - On new chapter creation:
     - Parse chapter outline → extract mentioned character names
   ...
   - No AI calls — pure heuristic (name matching, FK lookups)

7. **Post-Accept Entity Extraction (P1)** — AI Layer
...
- Semantic search / sqlite-vec embeddings → M7
- QualityService / consistency rules → M7
- Version history + diff viewer → M7
- CLI commands (search, check, config) → M7
- Ambient entity detection during writing → M8+
- Full chapter import + auto-extraction → M6
- Performance optimization / docs / release → M7
...
| 7 | **Post-Accept Extraction** | AI detects entities from accepted text, review → commit flow |
...
- [ ] SearchService implementation
  - [ ] `search(query, options?)` — FTS5 query with BM25 ranking
...
- [ ] Cmd+K search modal component
  ...
  - [ ] Click/Enter → navigate to entity detail or chapter
...
- [ ] Contextual relevance
  - [ ] Chapter FK arrays → highlight "in this chapter" entities
  - [ ] Outline mentions → highlight "in outline" entities
...
- [ ] Visualization component: `PacingView.tsx`
  ...
  - [ ] Chapter-type heatmap (action / dialogue / introspection — derived from outline tags or AI analysis)
- [ ] Interactions
...
- [ ] Suggestion UI
  - [ ] Entity chips with source indicator (📝 outline / ⬅️ prev chapter / 🎭 arc)
  - [ ] One-click to attach entity to chapter (updates chapter FK arrays)
  - [ ] "Dismiss" to hide suggestion
  - [ ] "Add more" → opens Bible panel search
...
- [ ] Background extraction trigger
  ...
  - [ ] Async call — does not block writing
...
- [ ] Keyboard shortcut system
  ...
  - [ ] Shortcut registry: action → key combo mapping
  - [ ] Cmd+K: search, Cmd+Enter: send to AI, Cmd+Shift+B: Bible, Cmd+Shift+O: outline, Cmd+S: save
  - [ ] Shortcut reference modal (Cmd+/)
- [ ] Integration polish
  - [ ] Bible panel ↔ search: Cmd+K within Bible panel filters entities
  - [ ] Visualization tabs: smooth transitions, loading states
  - [ ] Chapter setup ↔ Bible panel: "Add more" navigates correctly
  - [ ] Entity extraction ↔ Bible panel: newly created entities appear immediately
- [ ] Visual polish
...
### Integration Tests
- [ ] Search pipeline: create entities → search → verify results
- [ ] Bible panel: render all entity types, inject-to-context updates builder
- [ ] Chapter setup: create chapter with outline → verify suggestions match
- [ ] Post-accept extraction: accept AI text → extraction runs → review → commit → verify in DB
- [ ] Shortcut system: trigger shortcut → action fires

### E2E Tests
- [ ] Cmd+K → type query → select result → navigated to entity
- [ ] New chapter → outline filled → setup assist shows relevant entities → attach → AI uses them
- [ ] AI generate → accept → extraction detects entities → confirm → entities in Bible
...
-- UPDATE → update search_index
-- DELETE → delete from search_index
```

### Chapter Setup Assist — Heuristic Pipeline

```
New Chapter Created
       │
       ├─ Parse outline text → regex match known entity names
       │   (character names, location names from Bible)
       │
       ├─ Previous chapter → copy linked entity IDs
       │
       ├─ Current arc → arc.characters[]
       │
       └─ Deduplicate + rank
           │
           v
...
### Post-Accept Extraction — Option C Pipeline
...
User accepts AI text
  ...
  Async POST /api/intake/extract-entities
       │
       ├─ Gemini prompt: text + existing Bible names
       │
       ├─ Parse response → { mentioned[], new[] }
       │
       └─ Store in memory
           │
           v
    Sidebar notification ("3 entities detected")
           │
     User: review → confirm / edit / dismiss
           │
           v
    Confirmed → write to Story Bible repos
```
...
- M4 (Writing UX) complete — base writing features stable
- D3 or equivalent for force-directed graph
- Recharts or equivalent for charts
- Gemini 3.0 Pro API — for entity extraction
...
- 这是一个 **writing intelligence** milestone — 让写作体验从"工具"升级到"伙伴"
- Visualization 不要拖延（用户明确要求）— 功能性优先，视觉美化在 Phase 6
- Post-accept extraction 是 Smart Intake (M6) 的最小前置验证 — 如果 extraction pipeline 在这里跑通，M6 的 decomposition pipeline 会更顺利
- Product pivot: 写作从零开始是核心场景。Chapter Setup Assist 和 extraction 都假设用户在逐章创作，不是在导入已有作品
- 搜索只做 FTS5，semantic search 留给 M7 — 先让搜索有用，再让搜索聪明
- Relationship map 是用户最直观感知"这个工具懂我的故事"的 feature — 优先级不能降
